...
- Pré-condições: O utilizador está autenticado, o Intake encontra-se em `Draft` e os dados foram previamente guardados.
- Trigger: O ator principal seleciona a ação `Aprovar`.
- Pós-condições (sucesso): O estado do Intake muda para `Aprovado` e fica bloqueado a edições normais.
- Pós-condições (falha/cancelamento): O estado permanece `Draft` e o sistema apresenta a razão do bloqueio.
...
- Pré-condições: O utilizador está autenticado, o Intake encontra-se em `Aprovado` ou `Reaberto`, e o perfil possui acesso à ação de `Override`.
- Trigger: O ator principal seleciona a ação `Override`.
- Pós-condições (sucesso): O sistema autoriza a edição excecional, regista a justificação e mantém rasto de auditoria da ação.
- Pós-condições (falha/cancelamento): O override é cancelado e os dados aprovados permanecem sem alterações.
...
- Pré-condições: O utilizador está autenticado e o Intake encontra-se em `Aprovado`.
- Trigger: O ator principal seleciona a ação `Reabrir`.
- Pós-condições (sucesso): O estado do Intake muda para `Reaberto` e o processo volta a permitir edição autorizada.
- Pós-condições (falha/cancelamento): O estado do Intake mantém-se em `Aprovado` e a tentativa inválida é bloqueada.
...
## Notas orientadas pela variante (obrigatório)
