# Lab 14 — Matriz Principal de Rastreabilidade

## Objetivo

Consolidar a rastreabilidade do dossiê numa matriz única `REQ → AC → Test Case → Unit Test → Cenário BDD → Evidência`.
...
| Requisito | Critérios de Aceitação / comportamento rastreado | Test Case | Unit Test | Cenário BDD | Evidência | Estado |
| --- | --- | --- | --- | --- | --- | --- |
...
| REQ-003 — Restringir aprovação do Intake por RBAC | Roles autorizadas aprovam; `Viewer` recebe `Acesso Negado`; estado não muda quando bloqueado. | TC-003, TC-004, TC-005, TC-019 | `test_authorized_transition_manager_can_approve_valid_intake`; `test_authorized_security_officer_can_approve_valid_intake`; `test_viewer_cannot_approve_intake` | `Happy path - Transition Manager aprova um Intake válido`; `Percurso alternativo - Security Officer aprova um Intake válido`; `Negative path - Viewer tenta aprovar e é bloqueado` | `docs/unit_test_report.md`; `docs/bdd_report.md`; `docs/traceability_req_bdd.md` | Automatizado com PyTest e Behave |
